...
function Import-Module-Verified {
    Param (
        [Parameter(Mandatory=$true)]
        [string]$Name
    )
...
function Set-ItemProperty-Verified {
    ...
    if ($Name) {
        if ($Type) {
            Set-ItemProperty -Path "$Path" -Name "$Name" -Type $Type -Value $Value -Force -ErrorAction SilentlyContinue | Out-Null
        } else {
...
function Download-File {
	...
	# if (-not (Test-NetConnection www.google.com).PingSucceeded) { throw "No Internet Connection" }
